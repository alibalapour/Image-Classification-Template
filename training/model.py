...
def prepare_model(num_classes=2, model_name='ResNet'):
    """
    This function builds and returns a model
    """
    if model_name == 'ResNet':
        return ResNet(3, num_classes=num_classes, n=1)


# just for test :)
if __name__ == '__main__':
    # test 1
    net = prepare_model(num_classes=10, model_name='ResNet').to('cuda')
    print(summary(net, (3, 32, 32)))
    print('*' * 100)

    # test 2
    tensor = torch.rand((10, 3, 32, 32)).to('cuda')
    print('sample output: ', net(tensor))
    print('number of params :', sum(p.numel() for p in net.parameters()))
    print('*' * 100)

    # test 3
    macs, params = profile(net, inputs=(torch.rand((128, 3, 32, 32)).to('cuda'),))
    print(macs)
